...
export default async function DemographicsPage() {
  ...
  type BoothChartRow = {
    booth_number: number; male: number; female: number; total: number;
    bjp_share: number; sp_share: number; bsp_share: number;
    turnout_pct: number | null; lean: string; label: string;
  };
  ...
  booths.forEach((b) => {
    boothMap[b.booth_number] = {
      ...
      bjp_share: 0, sp_share: 0, bsp_share: 0, turnout_pct: null,
      lean: b.digital_lean_label ?? "INSUFFICIENT", label: `B${b.booth_number}`,
    };
  });
  ...
  const segments: { label: string; count: number; desc: string; color: string; sub: string }[] = [
    { label: "Women-Priority", count: booths.filter((b) => b.female_voters && b.male_voters && b.female_voters > b.male_voters).length,
      desc: "Female voters exceed male", color: "#ec4899", sub: `of ${booths.length} booths` },
    { label: "Strong BJP", count: leanCounts["STRONG_BJP"] ?? 0,
      desc: "BJP margin > 40 pts", color: "#f97316", sub: `of ${booths.length} booths` },
    { label: "Lean BJP", count: leanCounts["LEAN_BJP"] ?? 0,
      desc: "BJP margin 15–40 pts", color: "#fb923c", sub: `of ${booths.length} booths` },
    { label: "High Confidence", count: booths.filter((b) => b.confidence_label?.toUpperCase() === "HIGH").length,
      desc: "Signal confidence: HIGH", color: "#10b981", sub: `of ${booths.length} booths` },
    { label: "Avg Turnout > 60%", count: boothChartData.filter((b) => (b.turnout_pct ?? 0) > 60).length,
      desc: "Turnout above 60%", color: "#3b82f6", sub: `of ${booths.length} booths` },
    { label: "Low Turnout < 50%", count: boothChartData.filter((b) => b.turnout_pct != null && b.turnout_pct < 50).length,
      desc: "Turnout below 50%", color: "#ef4444", sub: `of ${booths.length} booths` },
  ];
  ...
  return (
    <div className="min-h-screen p-5" style={{ background: "var(--bg-base)" }}>
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <div className="flex items-center gap-2 text-xs mono mb-2" style={{ color: "var(--text-4)" }}>
            <span>Gorakhpur Urban AC</span>
            <ChevronRight size={10} />
            <span style={{ color: "var(--text-3)" }}>Demographics</span>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {[
          { label: "Registered Voters",  value: fmt(totalVoters),
            sub1: `${fmt(totalMale)} male`, sub2: `${fmt(totalFemale)} female`,
            icon: Users, color: "#3b82f6", bar: null },
          { label: "Gender Ratio",       value: genderRatio != null ? `${genderRatio}` : "—",
            sub1: "females per 1,000 males", sub2: `${pct(totalFemale, totalVoters)} female electorate`,
            icon: Users, color: "#ec4899", bar: genderRatio != null ? Math.min(100, (genderRatio / 1100) * 100) : null },
          { label: "2022 Turnout",       value: turnoutPct != null ? `${turnoutPct.toFixed(1)}%` : "—",
            sub1: turnout ? `${fmt(turnout.total_votes)} votes cast` : "—",
            sub2: turnout ? `of ${fmt(turnout.total_voters)} registered` : "—",
            icon: TrendingUp, color: "#10b981", bar: turnoutPct },
          { label: "BJP Vote Share 2022",value: bjpVoteShare != null ? `${bjpVoteShare.toFixed(1)}%` : "—",
            sub1: `${bjpTotal} of ${booths.length} booths BJP-leaning`,
            sub2: `${oppTotal} opp-leaning · ${leanCounts["NEUTRAL"] ?? 0} neutral`,
            icon: Shield, color: "#f97316", bar: bjpVoteShare },
          ...
          <div key={label} className="card p-5">
            ...
            <p className="text-2xl font-bold mb-0.5" style={{ color: "var(--text-1)" }}>{value}</p>
            <p className="text-xs" style={{ color: "var(--text-3)" }}>{sub1}</p>
            <p className="text-xs mt-0.5" style={{ color: "var(--text-4)" }}>{sub2}</p>
            {bar != null && (
              <div className="mt-3 h-1 rounded-full" style={{ background: "var(--bg-base)" }}>
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-3 mb-6">
        {segments.map((s) => (
          <div key={s.label} className="rounded-xl px-3.5 py-3"
            ...
            <p className="text-2xl font-bold mb-0.5" style={{ color: "var(--text-1)" }}>{s.count}</p>
            <p className="text-xs font-medium mt-0.5" style={{ color: s.color }}>{s.label}</p>
            <p className="text-xs mt-0.5" style={{ color: "var(--text-4)" }}>{s.desc}</p>
            <div className="mt-2 h-0.5 rounded-full" style={{ background: "var(--bg-surface)" }}>
              ...
              {boothChartData.map((b) => {
                ...
                return (
                  <tr key={b.booth_number}>
                    <td className="mono" style={{ color: "var(--text-4)" }}>{b.booth_number}</td>
                    <td className="font-medium truncate max-w-40" style={{ color: "var(--text-1)" }}>
                      {booth?.name ?? `Booth ${b.booth_number}`}
                    </td>
                    <td className="mono font-semibold" style={{ color: "var(--text-1)" }}>{fmt(b.total)}</td>
                    <td>
                      <div className="flex items-center gap-1.5">
                        <div className="w-8 h-1 rounded-full" style={{ background: "var(--bg-base)" }}>
                    ...
                    <td>
                      <span className="mono font-semibold"
                        style={{ color: fPct > 50 ? "#ec4899" : "var(--text-3)" }}>
                        {fPct.toFixed(1)}%
                      </span>
                    </td>
                    <td>
                      <div className="flex items-center gap-1.5">
                        <div className="w-10 h-1 rounded-full" style={{ background: "var(--bg-base)" }}>
