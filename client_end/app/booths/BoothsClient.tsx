    ...
    <div className="p-5 min-h-screen" style={{ background: "var(--bg-base)" }}>
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <div className="flex items-center gap-2 mb-0.5">
            ...
            <h1 className="font-bold" style={{ fontSize: 15, color: "var(--text-1)" }}>Booth Intelligence</h1>
      ...
      <div className="grid grid-cols-4 md:grid-cols-8 gap-3 mb-4">
        {[
          ...
          { label: "Total Voters",  value: fmt(totalVoters),           color: "var(--text-2)" },
      ...
      <div className="flex gap-3 mb-3">
        <div className="flex-1 relative">
          ...
          <input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }}
            ...
            className="w-full pl-8 pr-4 py-2 rounded-md text-xs outline-none"
            ...
            <thead>
              <tr>
                {[
                  ...
                  <th key={i}>
                    {key ? (
                      <button onClick={() => sort(key as SortKey)} className="flex items-center gap-1 transition-colors group">
                  ...
                  <tr key={b.booth_id}>
                    ...
                    <td>
                      <Link href={`/booths/${b.booth_id}`}
                        className="text-xs font-medium hover:text-orange-500 transition-colors line-clamp-1 max-w-40 block"
                    ...
                    <td className="mono text-xs" style={{ color: "var(--text-2)" }}>{fmt(b.total_voters)}</td>
                    <td>
                      {femalePct != null ? (
                        <div className="flex items-center gap-1.5">
                          <div className="w-14 h-1.5 rounded-full overflow-hidden" style={{ background: "var(--bg-base)" }}>
                          ...
                          <span className="mono text-xs" style={{ color: "var(--text-4)", fontSize: 9 }}>{femalePct.toFixed(0)}%F</span>
                        </div>
                    ...
                    <td>
                      {b.top_issue ? (
                        <span className="text-xs px-1.5 py-0.5 rounded mono capitalize"
                          style={{ background: "var(--bg-surface)", color: "var(--text-2)", border: "1px solid var(--border)", fontSize: 9 }}>
                          {b.top_issue.replace(/_/g, " ")}
                        </span>
                      ) : <span style={{ color: "var(--text-4)" }}>—</span>}
                    </td>
                    <td className="mono text-xs" style={{ color: "var(--text-2)" }}>
                      {b.event_count ?? 0}
                    </td>
