...
def task_booth_metrics() -> int:
    from analytics.booth_metrics import run as run_booth_metrics
...
def task_data_quality(window_days: int = 7) -> int:
    from analytics.data_quality import run_all_booths
    from api.db import get_pg_engine
...
def task_contradictions(window_days: int = 7) -> int:
    from analytics.contradiction_detector import run_all_booths
    from api.db import get_pg_engine
    n = run_all_booths(get_pg_engine(), window_days=window_days)
...
def task_scheme_gaps(window_days: int = 30) -> int:
    from analytics.scheme_gap_analysis import run_all_booths
    from api.db import get_pg_engine
    n = run_all_booths(get_pg_engine(), window_days=window_days)
...
def task_narratives(window_days: int = 7) -> int:
    from analytics.narrative_detector import run_all_booths
    from api.db import get_pg_engine
    n = run_all_booths(get_pg_engine(), window_days=window_days)
