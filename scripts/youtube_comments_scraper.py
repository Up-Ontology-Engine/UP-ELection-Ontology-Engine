...
class YouTubeCommentsScraper:
    ...
    def scrape_multiple_videos(self, video_ids: List[str], max_comments_per_video: int = 200) -> List[Dict]:
    ...
    def extract_mentions(self, comments: List[Dict]) -> Dict:
        ...
        # Sort by frequency
        entities['names'] = dict(sorted(entities['names'].items(), key=lambda x: x[1], reverse=True)[:20])
        entities['parties'] = dict(sorted(entities['parties'].items(), key=lambda x: x[1], reverse=True))
