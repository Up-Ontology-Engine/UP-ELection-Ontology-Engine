...
class YouTubeVideoScraper:
    ...
    def search_videos(self, query: str, max_results: int = 50, 
                     region_code: str = 'IN', publish_date_after: str = None) -> List[Dict]:
        """Search for videos using YouTube API"""
        if not self.use_api:
            logger.warning("YouTube API key not available. Install google-api-python-client and set YOUTUBE_API_KEY")
            return []
        ...
        try:
            request = self.youtube.search().list(
                ...
                type='video',
                order='relevance',
                publishedAfter=publish_date_after or (datetime.now() - timedelta(days=90)).isoformat() + 'Z',
