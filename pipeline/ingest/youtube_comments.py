...
def fetch_comments_ycd(
    ...
    try:
        ...
        logger.error(
            "youtube-comment-downloader not installed. Run: pip install youtube-comment-downloader"
        )
        return []
...
def classify_and_save(comments: list[dict]) -> Path:
    ...
    new = [c for c in classified if c.get("content_hash", c.get("comment_id", "")) not in seen]
    merged = existing + new
...
def classify_existing() -> Path:
    ...
    logger.info(
        f"Loaded {len(all_comments)} comments from {len(list(BY_VIDEO_DIR.glob('*_comments.json')))} videos"
    )
    if not all_comments:
...
def run(
    ...
    if classify:
        classified_path = max(COMMENTS_PROC_DIR.glob("comments_classified_*.json"), default=None)
        if classified_path:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--video", type=str, default=None, help="Single video URL")
    parser.add_argument(
        "--max-videos", type=int, default=831, help="Max videos to scrape (default: all 831)"
    )
    parser.add_argument(
        "--max-per-video", type=int, default=200, help="Max comments per video (default: 200)"
    )
    parser.add_argument("--classify", action="store_true", help="Classify comments after fetching")
    parser.add_argument(
        "--classify-existing",
        action="store_true",
    ...
    parser.add_argument(
        "--no-resume", action="store_true", help="Re-scrape even already-scraped videos"
    )
