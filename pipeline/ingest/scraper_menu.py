...
Search options:
  ...
  6. Back to menu
        ...
        for i, article in enumerate(results[:15], 1):
            sentiment_emoji = (
                "✅"
                if article.get("sentiment") == "Pro-BJP"
                else "❌"
                if article.get("sentiment") == "Anti-BJP"
                else "➖"
            )
...
def statistics_menu(collector):
    ...
    print(
        f"  ✅ Pro-BJP       : {pro_count:>4} articles ({pro_count*100//len(collector.articles):>2}%)"
    )
    print(
        f"  ❌ Anti-BJP      : {anti_count:>4} articles ({anti_count*100//len(collector.articles):>2}%)"
    )
    print(
        f"  ➖ Neutral       : {neutral_count:>4} articles ({neutral_count*100//len(collector.articles):>2}%)"
    )
...
def show_collection(collector):
    ...
    if len(collector.articles) > 5:
        try:
            show_count = int(
                input(f"\nShow how many articles? (max {len(collector.articles)}): ").strip()
                or "10"
            )
