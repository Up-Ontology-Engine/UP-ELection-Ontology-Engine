...
def _load_classified(directory: Path) -> list[dict]:
    ...
    for f in sorted(directory.glob("*.json")):
        try:
            ...
            records.extend(data.get("classified_articles") or data.get("videos") or [])
...
def run_analysis() -> None:
    ...
    yt_out.write_text(
        json.dumps({"generated_at": now, **yt_stats}, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    ...
    news_out.write_text(
        json.dumps({"generated_at": now, **news_stats}, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    ...
    comb_out.write_text(json.dumps(combined, ensure_ascii=False, indent=2), encoding="utf-8")
