...
def run(dry_run: bool = False, validate_only: bool = False,
    ...
    try:
        ...
        if lock_acquired and redis_client:
            try:
                redis_client.delete("lock:news_scraper")
                logger.info("Released Redis distributed lock 'lock:news_scraper'")
            except Exception as exc:
                logger.error(f"Failed to release Redis distributed lock: {exc}")
