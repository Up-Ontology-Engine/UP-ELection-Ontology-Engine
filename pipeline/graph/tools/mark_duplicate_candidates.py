...
def backup_postgres(engine: sa.Engine):
    with engine.connect() as conn:
        conn.execute(
            text(
                "CREATE TABLE IF NOT EXISTS candidate_master_audit AS TABLE candidate_master WITH NO DATA;"
            )
        )
...
def ensure_column(engine: sa.Engine):
    with engine.connect() as conn:
        conn.execute(
            text(
                "ALTER TABLE candidate_master ADD COLUMN IF NOT EXISTS is_deprecated BOOLEAN DEFAULT FALSE;"
            )
        )
...
def main():
    ...
    parser.add_argument(
        "--apply", action="store_true", help="Apply is_deprecated flags to Postgres"
    )
    ...
    print(
        f"Found {len(mapping)} duplicate groups with {total_dupes} total duplicate nodes. Report: {report}"
    )
