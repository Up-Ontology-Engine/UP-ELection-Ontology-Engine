...
def _count_nodes(session, label: str) -> int:
    """Count nodes of a given label. Label is validated against the whitelist."""
    label = _validate_label(label)
    # Labels cannot be parameterised in Cypher; we whitelist above instead.
    r = session.run(f"MATCH (n:{label}) RETURN count(n) AS c").single()  # noqa: S608
