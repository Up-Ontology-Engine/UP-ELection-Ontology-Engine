...
def run(window_days_pulse: int = 7, window_days_schemes: int = 30) -> dict:
    ...
    n_quality = task_data_quality(window_days=window_days_pulse, wait_for=[n_metrics])
    n_contra = task_contradictions(window_days=window_days_pulse, wait_for=[n_metrics])
    n_scheme = task_scheme_gaps(window_days=window_days_schemes, wait_for=[n_metrics])

    # Stage 4 — narrative detection reads contradiction_flags, must run after stage 3
    n_narrative = task_narratives(window_days=window_days_pulse, wait_for=[n_contra])

    # Stage 5 — Neo4j sync, must run after all compute stages
    neo4j_counts = task_load_neo4j(wait_for=[n_quality, n_contra, n_scheme, n_narrative])
