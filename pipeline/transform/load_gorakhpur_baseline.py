...
def normalise_booth_results(conn):
    ...
    for raw in dirty:
        ...
        if normalised is None:
            ...
            conn.execute(
                text("UPDATE booth_results SET party = :new WHERE party = :old"),
...
def load_ac_master(conn):
    ...
    conn.commit()
    count = conn.execute(
        text("SELECT COUNT(*) FROM ac_master WHERE district_name='Gorakhpur'")
    ).scalar()
