...
def load_csv() -> pd.DataFrame:
    ...
    gkp = df[
        (df["Constituency_No"] == CONST_NO) & (df["District_Name"].str.upper() == DISTRICT)
    ].copy()
    logger.info("Loaded %d GKP_322 rows from TCPD CSV (%d years)", len(gkp), gkp["Year"].nunique())
    return gkp
...
def run(engine: sa.Engine) -> dict[str, int]:
    ...
    for year, yr_df in df.groupby("Year"):
        ...
        for _, row in yr_sorted.iterrows():
            ...
            vote_share = (
                float(row["Vote_Share_Percentage"])
                if pd.notna(row["Vote_Share_Percentage"])
                else 0.0
            )
                    ...
                    """),
                        {
                            ...
                            "yr": year,
                            "inc": (
                                bool(row.get("Incumbent", False))
                                if pd.notna(row.get("Incumbent"))
                                else False
                            ),
                            "opp": position == 2,
                        },
            ...
            if not is_nota and cid:
                ...
                incumbent = (
                    bool(row.get("Incumbent", False)) if pd.notna(row.get("Incumbent")) else False
                )
