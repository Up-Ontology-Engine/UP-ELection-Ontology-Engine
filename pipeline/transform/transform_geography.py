...
Sources:
  data/processed/text/eci_electoral_roll_gorakhpur.json  → ac_master rows
  data/processed/Convert to xcel sheet/electoral_roll.xlsx        → booth_master + voter demographics
  data/processed/Convert to xcel sheet/electoral_roll (1).xlsx    → same, additional parts
...
def validate(engine: sa.Engine) -> None:
    with engine.connect() as conn:
        ...
        booth_count = conn.execute(
            text("SELECT COUNT(*) FROM booth_master WHERE ac_id = :ac"), {"ac": AC_ID}
        ).scalar()
        total_v = conn.execute(
            text("SELECT SUM(total_voters) FROM booth_master WHERE ac_id = :ac"), {"ac": AC_ID}
        ).scalar()
    logger.info(
        "[VALIDATE] ac_master: %d rows | booth_master GKP_322: %d booths | %d total voters",
        ac_count,
        booth_count,
        total_v or 0,
    )
