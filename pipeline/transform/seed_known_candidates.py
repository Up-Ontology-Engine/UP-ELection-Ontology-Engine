            ...
            """),
                {
                    ...
                    "inc": c["is_incumbent"],
                    "opp": c["is_primary_opp"],
                },
            )

            # Delete first (no unique constraint on candidate_id pre-migration-004)
            conn.execute(
                text("DELETE FROM candidate_affidavits WHERE candidate_id = :cid"),
                {"cid": c["candidate_id"]},
            )
            conn.execute(
                text("""
            ...
            """),
                {
                    ...
                    "age": aff["age"],
                    "edu": aff["education"],
                    "crim": aff["criminal_cases"],
                    "serious": aff["serious_cases"],
                    "assets": aff["total_assets"],
                    "liabs": aff["total_liabilities"],
                },
            )
