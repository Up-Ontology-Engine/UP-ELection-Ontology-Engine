...
def _classify_gap(status: str, beneficiary_count: int, positive_events: int, negative_events: int, total_events: int) -> tuple[str, str, str]:
    if status != "completed":
        return ("in_progress", "Scheme is still in progress — outcome cannot be assessed yet.", "LOW")
    if total_events < MIN_SENTIMENT_EVENTS:
        return ("no_data", "No sentiment signal found for this scheme. Consider targeted field surveys.", "LOW")
    ...
    if avg_sentiment < -0.1 and high_reach:
        return ("execution_gap", "Scheme completed and reached {beneficiary_count} beneficiaries but negative sentiment persists — quality or last-mile execution issue.", "HIGH")
    if avg_sentiment < -0.1 and not high_reach:
        return ("reach_gap", "Scheme completed but beneficiary count is low and sentiment is negative — scheme may not be reaching intended recipients.", "HIGH")
    if high_reach:
        return ("awareness_gap", "Scheme completed, reached beneficiaries, but no positive credit signal — awareness / communication gap.", "MEDIUM")
    return ("reach_gap", "Scheme completed but low reach with neutral sentiment — coverage may be insufficient.", "MEDIUM")


def get_scheme_gaps_for_booth(engine: Engine, booth_id: str, window_days: int = 30, computed_at: Optional[datetime] = None) -> list[dict]:
    ...
    with engine.connect() as conn:
        rows = conn.execute(
            ...
            """), {"booth_id": booth_id, "cutoff": computed_at - __import__("datetime").timedelta(days=window_days), "now": computed_at}).mappings().fetchall()
...
def run_all_booths(engine: Engine, window_days: int = 30) -> int:
    with engine.connect() as conn:
        booth_ids = [r[0] for r in conn.execute(text("SELECT DISTINCT bpm.booth_id FROM booth_panchayat_mapping bpm JOIN scheme_activity sa ON sa.panchayat_id = bpm.panchayat_id")).fetchall()]
