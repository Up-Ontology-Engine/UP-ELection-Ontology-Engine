...
def detect_narratives_for_booth(
    ...
    with engine.connect() as conn:
        mixed = conn.execute(
            text(
                "SELECT COUNT(*) AS n FROM contradiction_flags WHERE booth_id = :booth_id AND computed_at <= :now AND flag_label IN ('MIXED_SIGNALS', 'SWING_INDICATOR')"
            ),
    ...
    return sorted(
        [n for n in narratives if n["strength"] >= MIN_STRENGTH], key=lambda x: -x["strength"]
    )
