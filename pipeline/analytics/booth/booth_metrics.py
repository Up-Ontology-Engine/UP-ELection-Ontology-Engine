...
def _confidence_label(score: float, events: int) -> str:
    if events >= 100 and score >= 0.65:
        return "HIGH"
    if events >= 30 and score >= 0.50:
        return "MEDIUM"
    if events >= 10:
        return "LOW"
    return "INSUFFICIENT"
    ...
    with engine.connect() as conn:
        ...
        for booth_id, events in booth_events.items():
            ...
            bjp_weighted = sum(
                ...
                for e in events
                if e["entity"] in ("BJP", "Yogi Adityanath", "Narendra Modi")
            )
            bjp_weight = sum(
                e["final_confidence"] * sw(e)
                for e in events
                if e["entity"] in ("BJP", "Yogi Adityanath", "Narendra Modi")
            )
            ...
            opp_weighted = sum(
                e["final_polarity"] * e["final_confidence"] * sw(e)
                for e in events
                if e["entity"] in ("SP", "BSP", "Congress", "Akhilesh Yadav", "Mayawati")
            )
            opp_weight = sum(
                e["final_confidence"] * sw(e)
                for e in events
            ...
            data_conf = sum(
                e["final_confidence"] * e.get("geo_confidence", 0.5) for e in events
            ) / len(events)
