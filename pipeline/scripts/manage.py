...
def cmd_status(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if r:
            pong = r.ping()
            if pong:
                print(ok(f"Connected  {dim(os.environ.get('REDIS_URL', ''))}"))
                info_mem = r.info("memory")
                used_mem = info_mem.get("used_memory_human", "N/A")
                peak_mem = info_mem.get("used_memory_peak_human", "N/A")
                keys = len(r.keys("cache:*"))
                print(f"    {green(f'{used_mem:>7}')}  {dim('used memory')} (peak: {peak_mem})")
                print(f"    {green(f'{keys:>7,}')}  {dim('cached items')} (prefix: cache:*)")
            else:
                print(fail("Ping failed"))
        else:
            print(warn("Redis client not initialized (REDIS_URL not set?)"))
    except Exception as e:
        print(fail(f"Cannot connect to Redis: {e}"))
