...
Reads the exported MyNeta pipeline data (``data/Myneta/*.json``, produced by
...
Outputs:
  data/Myneta/myneta_graph.json          (canonical)
  client_end/public/myneta_graph.json    (served statically to the frontend)
...
PUBLIC_OUT = ROOT / "client_end" / "public" / "myneta_graph.json"
...
def _load_ddp_graph_builder():
    ...
    """
    try:
        from digital_democracy_pipeline.graph_builder import _short, _build_cooccurrence_edges
        return _short, _build_cooccurrence_edges
    except Exception:
        import importlib.util
        gb_path = ROOT / "digital-democracy-pipeline" / "src" / "digital_democracy_pipeline" / "graph_builder.py"
...
def build_graph(constituency_files: list[Path]) -> dict[str, Any]:
    ...
    for path in constituency_files:
        ...
        add_node(const_id, f"{ac_name} ({year})", "Constituency",
        ...
        for c in data.get("candidates", []):
            ...
            if criminal > 0:
                crim_id = add_node("criminal::flagged",
                                   "Criminal Cases Declared", "CriminalRecord")
...
def run(myneta_dir: Path = MYNETA_DIR) -> dict[str, Any]:
    ...
    logger.info("Wrote %s (%d nodes, %d edges)", out, graph["stats"]["total_nodes"], graph["stats"]["total_edges"])
