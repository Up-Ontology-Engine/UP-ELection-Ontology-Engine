    ...
    with engine.connect() as conn:
        booth_ids = [
            ...
            for r in conn.execute(
                text("""
                    SELECT DISTINCT bpm.booth_id
                    FROM booth_panchayat_mapping bpm
                    JOIN scheme_activity sa ON sa.panchayat_id = bpm.panchayat_id
                """)
            ).fetchall()
        ]
...
def get_scheme_gap(booth_id: str, engine: sa.Engine) -> list[dict]:
    """Legacy wrapper — returns get_scheme_gaps_for_booth result."""
    return get_scheme_gaps_for_booth(engine, booth_id)
...
if __name__ == "__main__":
    ...
    print(f"Scheme gap analysis done for {n} booths.")
