...
def render(ac_id: str, ac_name: str, api_url: str) -> None:
    ...
    if not events:
        ...
        st.code("Run: python -m ingestion.political_events  OR  populate the political_events table")
...
def _render_summary(all_events: list[dict], filtered: list[dict]) -> None:
    ...
    bjp_pos = sum(1 for e in filtered
...
def _render_sentiment_chart(events: list[dict]) -> None:
    ...
    fig.add_trace(go.Scatter(
        ...
        marker=dict(size=8, color=[
            "#2ecc71" if v > 0 else "#e74c3c" if v < 0 else "#95a5a6"
            for v in cumulative
        ]),
...
def _render_timeline(events: list[dict]) -> None:
    ...
    for e in sorted_events[:30]:
        ...
        si_color = {"positive": "#2ecc71", "negative": "#e74c3c", "mixed": "#f39c12"}.get(si, "#8b949e")
        ...
        if parties:
            for party in str(parties).split(","):
                p = party.strip()
                if p:
                    pc = {"BJP": "#FF6B35", "SP": "#3498db", "BSP": "#9b59b6"}.get(p.upper(), "#8b949e")
                    parties_html += (
...
def _render_demo(ac_name: str) -> None:
    ...
    demo_events = [
        {"event_type": "rally",   "event_date": "2024-01-15", "title": "CM Yogi Rally at Gorakhpur",
         "event_description": "Massive rally by Chief Minister ahead of election season",
         "parties_mentioned": "BJP", "sentiment_impact": "positive", "location_hint": "Gorakhpur Urban"},
        {"event_type": "protest", "event_date": "2024-01-08", "title": "Water Shortage Protest",
         "event_description": "Citizens protested acute water shortage in urban areas",
         "parties_mentioned": "SP", "sentiment_impact": "negative", "location_hint": "Rustampur"},
        {"event_type": "government_announcement", "event_date": "2024-01-12",
         "title": "AIIMS Gorakhpur Phase-II Announced",
         "event_description": "Government announces expansion of AIIMS Gorakhpur campus",
         "parties_mentioned": "BJP", "sentiment_impact": "positive", "location_hint": "Gorakhpur"},
        {"event_type": "controversy", "event_date": "2024-01-18",
         "title": "Paper Leak Controversy Resurfaces",
         "event_description": "Opposition raises paper leak issue, targets youth vote",
         "parties_mentioned": "SP,BSP", "sentiment_impact": "negative", "location_hint": ""},
        {"event_type": "scheme_launch", "event_date": "2024-01-20",
         ...
         "parties_mentioned": "BJP", "sentiment_impact": "mixed", "location_hint": ""},
        {"event_type": "protest", "event_date": "2024-01-22",
         "title": "Farmer Protest on Sugarcane Price",
         "event_description": "Farmers demand increase in sugarcane MSP, block highway briefly",
         "parties_mentioned": "SP", "sentiment_impact": "negative", "location_hint": "Khajani Road"},
    ]
