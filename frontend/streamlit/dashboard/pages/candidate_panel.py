...
from dashboard.components.war_room import info_bar, inject_css
...
def render(ac_id: str, _booth_id: str, api_url: str) -> None:
    ...
    candidates.sort(
        key=lambda c: (0 if c.get("is_incumbent") else 1 if c.get("is_primary_opp") else 2)
    )
...
def _render_candidate_card(c: dict) -> None:
    ...
    if completeness == "winner_runnerup_only":
        st.warning(
            "⚠️ **Partial Election Data**: Detailed non-winner breakdown is not yet finalized in the database. Omitted rows will show pending."
        )
...
""")
        ...
        with col_mov:
            ...
            if movable_items:
                ...
                for item in movable_items:
                    ...
                    if val > 0:
                        mov_rows.append(
                            {"Asset Type": item.get("item", "Other"), "Value": _fmt_amount(val)}
                        )
                if mov_rows:
        ...
        with col_imm:
            ...
            if immovable_items:
                ...
                for item in immovable_items:
                    ...
                    if val > 0:
                        imm_rows.append(
                            {"Property Type": item.get("item", "Other"), "Value": _fmt_amount(val)}
                        )
                if imm_rows:
                    ...
                    itr_rows.append(
                        {
                            ...
                            "Disclosed By": rel.capitalize(),
                            "Annual Income": _fmt_amount(inc),
                        }
                    )
            if itr_rows:
            ...
            for h in history:
                ...
                status_str = (
                    ...
                    if is_winner
                        ...
                        "Runner-up"
                        if h.get("result_position_label") == "runner_up"
                        else "Contested"
                    )
                )
    ...
    with tab_sentiment:
        ...
        if score is not None and mentions > 0:
            ...
            st.caption(
                "No digital signal yet. Keep collecting comments and news to see sentiment analytics."
            )
