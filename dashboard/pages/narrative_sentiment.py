...
from dashboard.components.war_room import inject_css, section, info_bar, PLOTLY_LAYOUT, PALETTE
...
def render(ac_id: str, ac_name: str, window_days: int, api_url: str) -> None:
    ...
    ranked = sorted(
        [{"type": k, "avg_strength": v["strength_sum"]/v["count"],
          "booth_count": v["count"], "evidence": v["evidence"]}
         for k, v in agg.items()],
        key=lambda x: x["avg_strength"], reverse=True
    )
...
def _render_top_row(ranked: list[dict]) -> None:
    ...
    with c1:
        ...
        if rising:
            for r in rising:
                meta = NARRATIVE_META.get(r["type"], {"icon": "🔹", "color": "#8b949e", "label": r["type"]})
                st.markdown(
    ...
    with c2:
        ...
        if declining:
            for r in declining:
                meta = NARRATIVE_META.get(r["type"], {"icon": "🔹", "color": "#95a5a6", "label": r["type"]})
                st.markdown(
...
def _render_emotion_breakdown(narratives: list[dict]) -> None:
    ...
    emotion_counts: dict[str, int] = {
        "anger": 0, "frustration": 0, "hope": 0, "support": 0, "neutral": 0
    }
    for n in narratives:
    ...
    # Dominant emotion
    dominant_emo = max(emotion_counts, key=lambda k: emotion_counts[k]) if emotion_counts else "neutral"
...
def _render_narrative_detail(narratives: list[dict]) -> None:
    ...
    for n in sorted(narratives, key=lambda x: x.get("strength", 0), reverse=True)[:8]:
        ...
        meta = NARRATIVE_META.get(nt, {"icon": "🔹", "color": "#8b949e", "label": nt.replace("_"," ").title()})
...
def _render_demo() -> None:
    ...
    demo_narratives = [
        {"narrative_type": "employment_crisis",  "strength": 0.78, "evidence_count": 142,
         "description": "Youth unemployment driving negative sentiment across urban booths",
         "top_issues": ["jobs", "price_rise"], "booth_number": 223},
        {"narrative_type": "anti_incumbency",    "strength": 0.65, "evidence_count": 98,
         "description": "Growing frustration with ruling party over water and jobs",
         "top_issues": ["water", "governance"], "booth_number": 185},
        {"narrative_type": "development_positive","strength": 0.52, "evidence_count": 67,
         "description": "Highway and infrastructure development perceived positively",
         "top_issues": ["roads"], "booth_number": 201},
        {"narrative_type": "price_rise_narrative","strength": 0.71, "evidence_count": 115,
         "description": "Inflation and essential commodities price complaints rising sharply",
         "top_issues": ["price_rise", "farmer"], "booth_number": 190},
        {"narrative_type": "scheme_success",     "strength": 0.44, "evidence_count": 55,
         "description": "PM Kisan and Ujjwala scheme beneficiaries expressing positive sentiment",
         "top_issues": ["farmer", "other"], "booth_number": 210},
    ]
