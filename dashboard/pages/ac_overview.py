...
def render(ac_id: str, ac_name: str, booths: list[dict], api_url: str) -> None:
    ...
    st.markdown(f"## 🏠 Constituency Overview")
    ...
    if not booths:
        ...
        st.code("uvicorn api.main:app --reload  +  python -m flows.graph.flow_load_graph --stage etl")
        return
...
def _render_summary_strip(df: pd.DataFrame) -> None:
    ...
    lean_bjp     = int((df.get("digital_lean_label", pd.Series(dtype=str)).str.contains("BJP", na=False)).sum())
    lean_opp     = int((df.get("digital_lean_label", pd.Series(dtype=str)).str.contains("Opp", na=False)).sum())

    overall_lean = ("Lean BJP" if bjp_avg > opp_avg + 0.05
                    else "Lean Opposition" if opp_avg > bjp_avg + 0.05
                    else "Contested")
...
def _render_historical_lean(df: pd.DataFrame) -> None:
    ...
    bjp_scores = df["bjp_pulse_score"].dropna() if "bjp_pulse_score" in df.columns else pd.Series(dtype=float)
    opp_scores = df["opp_pulse_score"].dropna() if "opp_pulse_score" in df.columns else pd.Series(dtype=float)

    fig = go.Figure()
    if len(bjp_scores) > 0:
        fig.add_trace(go.Histogram(
            x=bjp_scores, name="BJP",
            marker_color=PALETTE["bjp"], opacity=0.75,
            nbinsx=20,
        ))
    if len(opp_scores) > 0:
        fig.add_trace(go.Histogram(
            x=opp_scores, name="Opposition",
            marker_color=PALETTE["sp"], opacity=0.75,
            nbinsx=20,
        ))
...
def _render_issue_heatmap(df: pd.DataFrame) -> None:
    ...
    ISSUE_COLORS = {
        "Water": "#3498db", "Jobs": "#e74c3c", "Roads": "#95a5a6",
        "Electricity": "#f39c12", "Farmer": "#2ecc71", "Health": "#e67e22",
        "Education": "#9b59b6", "Corruption": "#e74c3c", "Price Rise": "#f39c12",
    }
...
def _render_booth_table(df: pd.DataFrame) -> None:
    ...
    display_cols = [c for c in [
        "booth_number", "name", "total_voters",
