...
def load_booths_to_postgres(booth_data: dict[str, dict], ac_no: int, engine: sa.Engine) -> int:
    """Upsert booth_master with voter demographics from PDF extraction."""
    ac_id   = f"GKP_{ac_no}"
    loaded  = 0
        ...
        for booth_id, d in booth_data.items():
            ...
            conn.execute(text("""
                ...
                ON CONFLICT (booth_id) DO UPDATE SET
                    ...
                    updated_at    = NOW()
            """), {
                "booth_id": booth_id, "ac_id": ac_id, "part_no": part_no,
                "male": d["male_voters"], "female": d["female_voters"],
                "other": d["other_voters"], "total": d["total_voters"],
            })
            loaded += 1
        conn.commit()

    logger.info("Upserted %d booths into booth_master (AC %d)", loaded, ac_no)
    return loaded
...
def run(pdf_paths: list[tuple[Path, int]] | None = None, dry_run: bool = False):
    ...
    for pdf_path, ac_no in targets:
        if not pdf_path.exists():
            ...
            continue
        booths = process_pdf(pdf_path, ac_no, engine, dry_run=dry_run)
