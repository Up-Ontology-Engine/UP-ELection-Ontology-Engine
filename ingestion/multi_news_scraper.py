...
def _fetch_raw_curl(url: str, timeout: int = 15) -> bytes:
    """Fallback fetcher using system curl (handles TLS edge-cases urllib can't)."""
    ...
    if result.returncode != 0:
        raise RuntimeError(f"curl failed (code {result.returncode}): {result.stderr.decode()[:200]}")
...
def _fetch_text(url: str, timeout: int = 15) -> str:
    try:
        ...
        raw = _fetch_raw_curl(url, timeout)
...
if __name__ == "__main__":
    ...
    print(f"\nDone. {len(articles)} unique articles collected.")
    if articles:
