...
Usage: python -m ingestion.news_scraper
"""
...
SOURCES: list[dict] = [
    {
        "name": "jagran",
        "url": "https://www.jagran.com/uttar-pradesh/gorakhpur.html",
...
def scrape_source(source: dict) -> Iterator[dict]:
    ...
    for card in soup.select(source["article_selector"])[:30]:
        ...
        if not href.startswith("http"):
            href = f"https://www.{source['name']}.com{href}"
...
def run():
    ...
    logger.info(f"News ingestion complete. Total: {total}")
